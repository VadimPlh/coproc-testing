...
$RPK_PATH topic create one_to_one -p ${PARTITIONS} -r 3 --brokers ${BROKERS}

for (( i=0; i < ${COPROC_COUNT}; i++ ))
do
    template=`cat ${SCRIPTS_DIR_PATH}/simple_template.js`
    new_code=${template//_input/one_to_one}
...
for (( i=0; i < 4; i++ ))
do
    $producer --topic "one_to_one" --record-size ${RECORD_SIZE} --producer-props asks=-1   bootstrap.servers=${BROKERS} --throughput ${THROUGHPUT} --num-records $MESSAGES_COUNT &> ${PRODUCER_DIR}/producer_${i}.txt &
    $producer --topic "one_to_one" --record-size ${RECORD_SIZE_NEW} --producer-props asks=-1   bootstrap.servers=${BROKERS} --throughput ${THROUGHPUT} --num-records $MESSAGES_COUNT &> ${PRODUCER_DIR}/producer_${i}.txt &
done
...
do
    ...
    $consumer --topic "one_to_one."'_simple_${i}_' --bootstrap-server ${BROKERS} --messages ${expected_messages_simple} &> ${CONSUMER_DIR}/consumer_simple${i}.txt --timeout ${TIMEOUT} &
    expected_messages_filter=$((${MESSAGES_COUNT}))
    $consumer --topic "one_to_one."'filter_${i}_' --bootstrap-server ${BROKERS} --messages ${expected_messages_filter} &> ${CONSUMER_DIR}/consumer_filter${i}.txt --timeout ${TIMEOUT} &
done